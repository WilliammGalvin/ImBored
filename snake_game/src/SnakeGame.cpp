...
void snake_game::SnakeGame::renderGrid(sf::RenderTarget& target) {
    ...
    for (int y = 0; y < _gridDim; y++) {
        for (int x = 0; x < _gridDim; x++) {
            ...
            const float totalBoardWidth = _tileSize * static_cast<float>(_gridDim);

            const auto offset = sf::Vector2f{
                static_cast<float>(window->getWindowSize().x) - totalBoardWidth,
                static_cast<float>(window->getWindowSize().y) - totalBoardWidth,
            } / 2.0f;
